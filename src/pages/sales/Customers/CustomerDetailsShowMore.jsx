...
const CustomerDetailsShowMore = memo(({ baseUrl }) => {
  ...
  const renderContent = () => {
    ...
    if (status === "error") {
      return <InfiniteScrollSelectError error={error} handleRetry={handleRetry} />;
    }
    return (
      data?.pages?.map((page) => (
        <React.Fragment key={page.nextPageNumber ?? 1}>
          {page.results?.map((item) => (
            ...
            >
              <Tooltip
                title={item.full_name?.length > 25 ? item.full_name : ""}
              >
...
CustomerDetailsShowMore.displayName = 'CustomerDetailsShowMore';const styles = {
  menuItem: {
    lineHeight: "20px",
    margin: "0rem",
    padding: " 5px 15px",
