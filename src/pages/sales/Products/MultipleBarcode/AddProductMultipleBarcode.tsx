...
const AddProductMultipleBarcode: React.FC<IProps> = (props) => {
  ...
  const showModal = async () => {
    if (props?.units?.length > 0) {
      setVisible(true);
      if (props?.barcodeList?.length > 0) {
        const newBarcodeList = props?.barcodeList?.map(
      ...
      message.error(t('Sales.Product_and_services.Form.Select_unit_error'));
    }
  };
  ...
  return (
    <div>
      <MedicineBoxOutlined onClick={showModal} />
      ...
      <Modal
        ...
        title={
          <ModalDragTitle
            ...
            title={t('Sales.Product_and_services.Form.Multiple_barcode')}
          />
        }
        modalRender={(modal) => (
        ...
        open={visible}
        width={i18n.language === 'en' ? 690 : 670}
...
export default AddProductMultipleBarcode;
interface IStyles {
