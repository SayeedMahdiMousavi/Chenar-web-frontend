...
const JournalBookTable: React.FC<IProps> = ({
  baseUrl = 'accounting_reports/financial/journal/',
  resultUrl = 'accounting_reports/financial/journal/result/',
  place = '',
}) => {
  ...
  const handleGetJournal = React.useCallback(
    ...
    async ({ queryKey }) => {
      ...
      const { data } = await axiosInstance.get(
        `${baseUrl}?page=${page}&page_size=${pageSize}&ordering=${order}&search=${search}&account_type=${accountTypeValue}&currency=${currencyId}&date_time_after=${newDate}&account=${bankId}`,
      );
      return data;
    },
    [baseUrl],
  ...
  const getJournalTotal = React.useCallback(
    ...
    async ({ queryKey }) => {
      ...
      const { data } = await axiosInstance.get(
        `${resultUrl}?currency=${currencyId}&account_type=${accountTypeValue}&date_time_after=${newDate}&account=${bankId}`,
      );
      console.log('result journal book ', data);
      return data;
    },
    [resultUrl],
  );
  ...
  const result = useQuery(
    [resultUrl, { currencyId, startDate, endDate, bankId, accountTypeValue }],
    getJournalTotal,
    {
      enabled: !!startDate,
      // cacheTime: 0,
  ...
  return (
    <>
      <Title level={2} style={{ padding: '1rem 0 0.5rem' }}>
