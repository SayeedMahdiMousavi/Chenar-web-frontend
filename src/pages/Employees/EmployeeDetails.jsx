...
import EmployeeDetailsEmployeeList from "./EmployeeDetailsEmployeeList";
...
function EmployeeDetails(props) {
  ...
  const handleGetItem = async ({ queryKey }) => {
    const { id } = queryKey?.[1] || {};
    const { data } = await axiosInstance.get(`${baseUrl}${id}/?expand=*`);
    return data;
  };

  const result = useQuery(
    [`${baseUrl}item/`, { id: params?.id }],
    handleGetItem,
    {
      enabled: !!params?.id,
    }
  ...
  const data1 = [
    {
      ...
      content: result?.data?.position,
    },
    {
      title: t("Sales.Customers.Form.National_id_number"),
      content: result?.data?.national_id_number,
    },
    ...
    },
    {
      ...
      content: result?.data?.full_billing_address
  ...
  const editAttachment = async (value) => {
    await axiosInstance
      .patch(`${baseUrl}${props.match.params.id}/`, value, {
        timeout: 0,
  ...
  const { mutate: mutateEditAttachment, isLoading } = useMutation(
    editAttachment,
    {
      onSuccess: () => {
        queryClient.invalidateQueries(`${baseUrl}`);
        queryClient.invalidateQueries(`${baseUrl}item/`);
                ...
                <Row gutter={[0, 10]} style={styles.sidebar}>
                  <Col span={24}>
                    <Row justify="space-between">
                      <Col span={19}>
                        <PageBackIcon
                          previousPageName={t("Employees.1")}
                          url={EMPLOYEE}
                        />
                      </Col>
                  ...
                  <Col sm={24} xs={{ span: 24 }}>
                    <EmployeeDetailsEmployeeList baseUrl={baseUrl} />
                  </Col>
                </Row>
              </Col>
              <Col>
              ...
              <Row>
                <Col span={24}>
                  ...
                  >
                    ...
                    <TabPane tab={t("Employees.Employee_details")} key="2">
                      <Row
                        gutter={{
                          xl: 50,
                          lg: collapse ? 35 : 50,
                        ...
                        {result?.data?.status === "active" && (
                          <Col span={24}>
                            <Row justify="end">
                              ...
                              >
                                {" "}
                                {checkPermissions(`change_${EMPLOYEE_M}`) && (
                                  <EditEmployee
                                    record={result && result?.data}
                                    attachment={fileList?.[0]?.name}
                                    baseUrl={baseUrl}
