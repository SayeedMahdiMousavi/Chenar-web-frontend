...
export const useCreateSupplierCategory = () => {
  ...
  return useGenericMutation(
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: [{ queryKey: ['suppliers', 'categories'] }] });
...
export const useUpdateSupplierCategory = () => {
  ...
  return useGenericMutation(
    ({ id, data }: { id: string | number; data: { name?: string; description?: string } }) =>
...
export const useSupplierPayments = (supplierId: string | number, params?: {
  page?: number;
  page_size?: number;
  date_from?: string;
  date_to?: string;
...
export const useSupplierInvoices = (supplierId: string | number, params?: {
  page?: number;
  page_size?: number;
  date_from?: string;
  date_to?: string;
...
export const useImportSuppliers = () => {
  ...
  return useGenericMutation(
    ...
    {
      onSuccess: (data: any) => {
        queryClient.invalidateQueries({ queryKey: [{ queryKey: ['suppliers'] }] });
        message.success(`Successfully imported ${data.imported_count} suppliers`);
...
export const useBulkDeleteSuppliers = () => {
  ...
  return useGenericMutation(
    (ids: number[]) => {
      return Promise.all(
        ids.map(id => supplierService.deleteSupplier(id))
      );
    },
    {
      onSuccess: () => {
...
export const useSupplierSearch = (searchTerm: string, options?: {
  category?: number;
  is_active?: boolean;
}) => {
  return useGenericQuery(
...
export const useActiveSuppliers = () => {
  return useGenericQuery(
    ...
    }),
    {
      ...
      select: (data: any) => data.results?.filter((supplier: any) => supplier.is_active),
    }
  );
};
