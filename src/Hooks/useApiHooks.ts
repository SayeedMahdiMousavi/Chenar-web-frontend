import { useMutation, useQuery, useQueryClient, UseQueryOptions, UseMutationOptions } from 'react-query';
...
export const createQueryKeys = (baseKey: string) => ({
  ...
  list: (params?: any) => [...createQueryKeys(baseKey).lists(), params] as const,
  details: () => [...createQueryKeys(baseKey).all, 'detail'] as const,
  detail: (id: string | number) => [...createQueryKeys(baseKey).details(), id] as const,
  infinite: (params?: any) => [...createQueryKeys(baseKey).all, 'infinite', params] as const,
});
...
export function createCRUDHooks<T, TCreateData, TUpdateData = Partial<TCreateData>>(
  service: ApiService,
  baseKey: string
) {
  ...
  // List hook
  const useList = (params?: PaginationParams, options?: Omit<UseQueryOptions<any>, 'queryKey' | 'queryFn'>) => {
  ...
  // Detail hook
  const useDetail = (id: string | number, options?: Omit<UseQueryOptions<T>, 'queryKey' | 'queryFn'>) => {
    return useGenericQuery(
      queryKeys.detail(id),
  ...
  // Update mutation
  const useUpdate = (options?: UseMutationOptions<T, Error, { id: string | number; data: TUpdateData }>) => {
    const queryClient = useQueryClient();
    
    return useGenericMutation(
      ({ id, data }: { id: string | number; data: TUpdateData }) => service.update<T, TUpdateData>(id, data),
      {
        onSuccess: (data, variables, context) => {
  ...
  // Delete mutation
  const useDelete = (options?: UseMutationOptions<any, Error, string | number>) => {
    const queryClient = useQueryClient();
    
    return useGenericMutation(
      (id: string | number) => service.deleteById(id),
      {
        onSuccess: (data, variables, context) => {
  ...
  const useSetStatus = (options?: UseMutationOptions<T, Error, { id: string | number; status: 'active' | 'deactivate' }>) => {
    const queryClient = useQueryClient();
    
    return useGenericMutation(
      ({ id, status }: { id: string | number; status: 'active' | 'deactivate' }) => service.setStatus<T>(id, status),
      {
        onSuccess: (data, variables, context) => {
          // Invalidate and update relevant queries
          queryClient.invalidateQueries(queryKeys.lists());
          ...
          const actionText = variables.status === 'active' ? 'activated' : 'deactivated';
